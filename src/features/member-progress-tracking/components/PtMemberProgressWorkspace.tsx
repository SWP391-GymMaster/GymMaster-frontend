...
type PtMemberProgressWorkspaceProps = {
  memberId: number
}

export function PtMemberProgressWorkspace({ memberId }: PtMemberProgressWorkspaceProps) {
  const progressQuery = useMemberProgress(memberId)
  const memberQuery = useMember360Data(memberId)

  const entries = progressQuery.data ?? []
  const member = memberQuery.data?.member

  const isLoading = progressQuery.isLoading || memberQuery.isLoading
  const error = progressQuery.error || memberQuery.error
  ...
  return (
    <PermissionGuard allowedRoles={["pt"]}>
      <WorkspaceShell
        description={`Xem biểu đồ và lịch sử thay đổi cân nặng, tỷ lệ mỡ của hội viên ${memberName}.`}
        role="pt"
        title={`Tiến độ hội viên: ${memberName}`}
      >
        <div className="space-y-6">
          {/* Top action rail */}
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="text-sm text-muted-foreground flex items-center gap-2">
              <Link href={`/pt/members/${memberId}`} className="hover:text-foreground flex items-center gap-1 transition">
                <ArrowLeft className="size-4" />
                Hồ sơ Member 360°
              </Link>
              <span>/</span>
              <span className="font-medium text-foreground">Tiến độ cơ thể</span>
            </div>
          </div>
            ...
            <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <h2 className="mt-4 text-3xl font-semibold tracking-tight text-foreground md:text-4xl">
                  Theo dõi tiến độ: {memberName}
                </h2>
                <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
                  Phân tích chỉ số cơ thể để đánh giá mức độ hấp thụ dinh dưỡng, phục hồi và điều chỉnh giáo án tập luyện phù hợp.
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:min-w-[400px]">
                <div className="rounded-xl border border-border bg-background p-4">
                  ...
                  <p className="mt-2 text-2xl font-bold text-foreground" data-testid="pt-latest-weight">
                    {weightVal}
                  </p>
                ...
                <div className="rounded-xl border border-border bg-background p-4">
                  ...
                  <p className="mt-2 text-2xl font-bold text-foreground" data-testid="pt-latest-fat">
                    {fatVal}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
          ...
          {isLoading ? (
            <StateBlock tone="loading" title="Đang tải dữ liệu tiến độ hội viên..." />
          ) : error ? (
            <div className="space-y-4">
              <StateBlock
                tone="error"
                title="Không thể tải dữ liệu tiến độ hội viên"
